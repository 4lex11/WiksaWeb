...
function cargarProductosCarrito(){
    if(productosEnCarrito && productosEnCarrito.length > 0){
        ...
        productosEnCarrito.forEach(producto => {
            ...
            div.innerHTML= `
                <span tabindex="0">${producto.name}</span>
                <span tabindex="0">Cantidad: ${producto.cantidad}</span>
                <span tabindex="0">Precio: ${producto.price} soles</span>
                <div>
                    <span tabindex="0">Subtotal: ${producto.price *producto.cantidad} soles</span>
                    <button id="${producto.id}" class="eliminar_producto" aria-label="Eliminar Item"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
...
function actualizarTotal(){
    ...
    contentMontoTotal.innerHTML = `Monto Total: ${montoTotal} soles`;
}
